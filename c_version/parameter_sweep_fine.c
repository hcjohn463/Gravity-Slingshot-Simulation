...
int main() {
    printf("🚀 Slingshot Parameter Sweep (C Version - Fine Grid)\n");
    printf("===================================================\n");
    ...
    // Open output file
    FILE *output_file = fopen("successful_parameters_fine.txt", "w");
    if (!output_file) {
    ...
    // Write header
    fprintf(output_file, "# Successful Slingshot Parameters (C Version - Fine Grid)\n");
    ...
    // Parameter ranges - Fine grid
    int vx_min = -4000, vx_max = -1000, vx_step = 1;  // Step size = 1 m/s
    int vy_min = 1000, vy_max = 4000, vy_step = 1;    // Step size = 1 m/s
    ...
    printf("This will take significantly longer due to fine grid!\n");
    printf("Progress: ");
    fflush(stdout);
    ...
    for (int vx = vx_min; vx <= vx_max; vx += vx_step) {
        for (int vy = vy_min; vy <= vy_max; vy += vy_step) {
            ...
            if (test_count % (total_tests / 100) == 0 || test_count == total_tests) {
                printf(".");
                fflush(stdout);
            }
        }
    ...
    printf("💾 Results saved to 'successful_parameters_fine.txt'\n");
    
    // Reopen file to add execution time at the end of header
    FILE *temp_file = fopen("temp_fine.txt", "w");
    FILE *read_file = fopen("successful_parameters_fine.txt", "r");
    if (temp_file && read_file) {
        char line[256];
        
        // Copy header and add execution time
        fprintf(temp_file, "# Successful Slingshot Parameters (C Version - Fine Grid)\n");
        fprintf(temp_file, "# Format: Vx(m/s), Vy(m/s), Speed(m/s), HitTime(steps)\n");
        fprintf(temp_file, "# Target: [4e7, 3e7] km, Radius: 100,000 km\n");
        fprintf(temp_file, "# Spacecraft initial position: [3e7, -2e7] km\n");
        fprintf(temp_file, "# Grid resolution: 1 m/s step size\n");
        fprintf(temp_file, "# Execution time: %.2f seconds\n\n", execution_time);
        
        // Skip original header lines and copy data
        ...
        remove("successful_parameters_fine.txt");
        rename("temp_fine.txt", "successful_parameters_fine.txt");
    }
